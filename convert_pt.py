...
pipe = StableDiffusion3Pipeline.from_pretrained("/data/aigc/models/stable-diffusion-3-medium-diffusers", torch_dtype=torch.float32)
...
# torch.onnx.export(vae_decoder, fake_inputs, "/data/aigc/demos/sd3models/vae/vae_decoder.onnx")
...
# if save_onnx: torch.onnx.export(sd3head, tuple(fake_inputs), "/data/aigc//demos/sd3models/mmdit/head.onnx")
...
for i in tqdm(range(23)):
    ...
    torch.jit.trace( sd3block[-1], (hidden_states, temb, encoder_hidden_states)).save(f"/data/aigc/demos/sd3models/mmdit/block_{i}.pt")
    encoder_hidden_states, hidden_states = sd3block[-1](hidden_states, temb, encoder_h
                                                        idden_states)
    # print(encoder_hidden_states)

last_block = SD3Block(23).eval()
if save_onnx:
    torch.jit.trace( last_block, (hidden_states, temb, encoder_hidden_states)).save(f"/data/aigc/demos/sd3models/mmdit/block_23.pt")
...
if save_onnx: 
    torch.jit.trace( sd3tail, (hidden_states, temb)).save(f"/data/aigc/demos/sd3models/mmdit/tail.pt")
...
torch.jit.trace(t5block0, input1).save("/data/aigc/demos/sd3models/t5/t5_encoder_block0.pt")
...
for i in tqdm(range(1,24)):
    ...
    torch.jit.trace(block, input1).save(f"/data/aigc/demos/sd3models/t5/t5_encoder_block{i}.pt")
    hidden_states = block(hidden_states)
    print(i, hidden_states.max(), hidden_states.min() )

torch.jit.trace(t5head, t5_encoder_inputs).save("/data/aigc/demos/sd3models/t5/t5_encoder_head.pt")
torch.jit.trace(t5model.encoder.final_layer_norm, hidden_states).save("/data/aigc/demos/sd3models/t5/tail.pt")
...
class CLIPHead(torch.nn.Module):
    ...
    def forward(self, x):
        return self.emb(x)
head = CLIPHead()
torch.onnx.export(head, test_input, "/data/aigc/demos/sd3models/clip_l/head.onnx")
...
for i in range(32):
    block0 = CLIPBlock(i)
    torch.onnx.export(block0, input1, f"/data/aigc/demos/sd3models/clip_l/block_{i}.onnx")
    input1 = block0(input1)
res_t = clip0_model.text_model.final_layer_norm(input1)
torch.onnx.export(clip0_model.text_model.final_layer_norm, input1, f"/data/aigc/demos/sd3models/clip_l/tail.onnx")
...
class CLIPHead(torch.nn.Module):
    ...
    def forward(self, x):
        return self.emb(x)
head = CLIPHead()
torch.onnx.export(head, test_input, "/data/aigc/demos/sd3models/clip_g/head.onnx")
input1 = head(test_input)
class CLIPBlock(torch.nn.Module):
    def __init__(self,idx):
        super().__init__()
...
for i in range(12):
    block0 = CLIPBlock(i)
    torch.onnx.export(block0, input1, f"/data/aigc/demos/sd3models/clip_g/block_{i}.onnx")
    input1 = block0(input1)
res_t = clip0_model.text_model.final_layer_norm(input1)
torch.onnx.export(clip0_model.text_model.final_layer_norm, input1, f"/data/aigc/demos/sd3models/clip_g/tail.onnx")
# ================= export over ==================== #