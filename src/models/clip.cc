...
int clip_pooling_run(struct clip_pooling *encoder, void* input_tokens, void* pooling_embed);
...
struct clip_pooling * clip_pooling_init(const char* filename, int device_id)
{
    struct clip_pooling *encoder = (struct clip_pooling *)calloc(1, sizeof(struct clip_pooling));
    ...
    DEVICE_ASSERT(bmrt_load_bmodel(encoder->model, filename)==true);
    encoder->net_head  = bmrt_get_network_info(encoder->model, "clip_head");
    encoder->net_block = bmrt_get_network_info(encoder->model, "clip_block_0");
    encoder->net_tail  = bmrt_get_network_info(encoder->model, "clip_tail");
    DEVICE_ASSERT( bm_malloc_device_byte(encoder->handle, &encoder->input_tokens,  encoder->net_head->max_input_bytes[0] ) == 0 );
    DEVICE_ASSERT( bm_malloc_device_byte(encoder->handle, &encoder->hidden,        encoder->net_head->max_output_bytes[0]) == 0 );
    DEVICE_ASSERT( bm_malloc_device_byte(encoder->handle, &encoder->pooling_embed, encoder->net_tail->max_output_bytes[0]) == 0 );
    return encoder;
}
...
{
    FUNC_TIME_START;
    DEVICE_ASSERT( bm_memcpy_s2d(encoder->handle, encoder->input_tokens, input_tokens) == 0 );
    ...
    DEVICE_ASSERT(ret);
    bm_thread_sync(encoder->handle);
    DEVICE_ASSERT( bm_memcpy_d2s(encoder->handle, pooling_embed, encoder->pooling_embed) == 0 );
